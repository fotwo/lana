    ...
    def __call__(self, inputs, is_training = False, reuse = False, scope = None):
        ...
        with tf.variable_scope(scope or type(self).__name__, reuse=reuse):
            ...
            if self.layerwise_init:
                 ...
                 initLastLayerOp = tf.variables_initializer(tf.get_collection(tf.GraphKeys.VARIABLES, scope=tf.get_variable_scope().name + '/layer' + str(self.num_layers)))
...
class NnetTrainer(object):
    ...
    def __init__(self, nnetGraph, input_dim, init_learning_rate, learning_rate_decay, num_steps, numframes_per_batch):
        ...
        with self.graph.as_default():
            ...
            with tf.name_scope('train'):
                ...
                self.initgrads = tf.variables_initializer(grads)
            ...
            self.summary = tf.summary.merge([tf.summary.histogram(val.name, val) for val in params+meangrads] + [tf.summary.scalar('loss', self.average_loss)])
    ...
    def computeLoss(self, targets, logits):
        return tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=targets, name='loss')
    ...
    def startVisualization(self, logdir):
        self.summarywriter = tf.summary.FileWriter(logdir=logdir, graph=self.graph)
