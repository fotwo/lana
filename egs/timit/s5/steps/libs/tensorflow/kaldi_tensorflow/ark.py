...
class ArkWriter(object):
    ...
    def write_next_utt(self, ark_path, utt_id, utt_mat):
        ...
        ark_file_write.write(struct.pack('<%ds'%(len(utt_id)), bytes(utt_id,'utf-8')))
        pos = ark_file_write.tell()
        ark_file_write.write(struct.pack(bytes('<xcccc','utf-8'),bytes('B','utf-8'),bytes('F','utf-8'),bytes('M','utf-8'),bytes(' ','utf-8')))
        ark_file_write.write(struct.pack(bytes('<bi','utf-8'), 4, rows))
        ark_file_write.write(struct.pack(bytes('<bi','utf-8'), 4, cols))
